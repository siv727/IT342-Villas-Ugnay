...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        if (mounted && res?.data) {
          const body = res.data;
          setProduct(body.success ? body.data : body);
        }
      } catch {
    ...
    const fetchMore = async () => {
      ...
      try {
        ...
        if (mounted && res?.data) {
          ...
          setMoreProducts((list as Product[]).filter((p) => p.id !== product.id && p.active !== false).slice(0, 4));
        }
      } catch {
  ...
  // Support SDD imageUrls array or legacy single image
  const mainImage = (product as Product & { imageUrls?: string[] }).imageUrls?.[0] || product.image;
  ...
  return (
    <div>
      <div className="flex items-center gap-1 text-xs text-neutral-400 mb-4">
        ...
        {manufacturer && (
          <>
            <Link to={`/vendor/manufacturer/${manufacturer.id}`} className="hover:text-primary">
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-12">
        <div className="lg:w-[55%] shrink-0">
          <img src={mainImage} alt={product.name} className="w-full h-[420px] object-cover rounded-2xl shadow-md" />
        </div>

        <div className="lg:w-[45%] flex flex-col">
          <h1 className="text-2xl lg:text-[28px] font-bold text-neutral-900 mb-2">{product.name}</h1>
          <p className="text-2xl font-bold text-primary mb-1">₱ {product.price.toFixed(2)}</p>
          ...
          {manufacturer && (
            <div className="text-sm text-neutral-700 mb-4">
              ...
              <Link to={`/vendor/manufacturer/${manufacturer.id}`} className="text-primary hover:underline">
                {manufacturer.businessName}
              </Link>
            </div>
          )}
          ...
          <div className="mt-auto flex flex-col gap-3">
            ...
            {manufacturer && (
              <Link to={`/vendor/manufacturer/${manufacturer.id}`}>
                <Button variant="secondary" fullWidth>View Manufacturer</Button>
              </Link>
            )}
          </div>
        </div>
            ...
            {moreProducts.map((p) => (
              <Card key={p.id} hover className="p-0 overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm font-semibold text-primary mb-2">₱ {p.price.toFixed(2)} / {p.unit}</p>
