...
export default function ManufacturerProfile() {
  ...
  const initials = (form.businessName || 'M')
    .split(' ')
    .map((w) => w[0])
    .join('')
    .slice(0, 2)
      ...
      <Card className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="lg:w-[30%] flex flex-col items-center gap-4">
          <div className="relative group">
            <div className="w-28 h-28 rounded-full bg-accent flex items-center justify-center text-white text-3xl font-bold">
              {initials}
            </div>
            <button
              type="button"
          ...
          <h2 className="text-lg font-semibold text-neutral-900 text-center">
            {form.businessName || 'Manufacturer'}
          </h2>
          <span className="text-xs px-3 py-1 rounded-full bg-accent-light text-accent font-medium capitalize">
