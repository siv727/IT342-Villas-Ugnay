...
export default function ProductManagement() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        if (mounted && res?.data) setProducts((res.data as Product[]).filter((p) => p.manufacturerId === 101));
      } catch {
  ...
  const handleToggle = async (product: Product) => {
    try {
      await productApi.updateProduct(product.id, { active: !product.active });
  ...
  // use shared Pagination component from UI
    ...
    <div>
      ...
      {paginated.length === 0 ? (
        <EmptyState
          ...
          description="Add your first product to start receiving sample requests."
          actionLabel="Add Product"
          actionTo="/manufacturer/products/add"
        />
      ) : (
