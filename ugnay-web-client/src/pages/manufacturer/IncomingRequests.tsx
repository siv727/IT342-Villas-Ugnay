...
export default function IncomingRequests() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const manufacturerId = user?.userId ?? 101;
        const res = await sampleRequestApi.getSampleRequests({ manufacturerId });
    ...
    <div>
      ...
      {paginated.length === 0 ? (
        <EmptyState
          ...
          description={statusFilter !== 'All' ? `No ${statusFilter.toLowerCase()} requests.` : 'No incoming requests yet.'}
        />
      ) : (
            ...
            <Link key={req.id} to={`/manufacturer/requests/${req.id}`}>
              <Card hover className="flex flex-col sm:flex-row sm:items-center gap-4 mb-2">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm font-semibold text-neutral-900">₱ {req.total.toFixed(2)}</span>
